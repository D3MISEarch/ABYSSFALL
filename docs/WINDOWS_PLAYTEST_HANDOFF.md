...
5. Press F8 after reproducing a problem to save a timestamped playtest report and copy its text to the Windows clipboard.
...
F8 reports are saved in the Godot user-data playtest_reports folder. The F3 panel displays the exact absolute path after a successful save. On a graphical desktop build, the same report text is copied to the clipboard so it can be pasted directly into ChatGPT, Claude, or a GitHub issue. Each report includes build identity, scene, FPS, active input profile, connected controller names, and a blank player-notes section.
...
- Paste the F8 report from the clipboard, or attach the saved report file
