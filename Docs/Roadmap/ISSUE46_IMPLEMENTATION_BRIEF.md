...
- prerequisite, rank, and cost validation;
- atomic spending;
...
- Unknown, negative, over-ranked, unaffordable, or prerequisite-invalid allocations must not be silently accepted.
...
- one mutually exclusive or tradeoff rule only if it can be implemented without widening the slice;
...
8. Close and immediately resume play.
9. Save, quit fully, relaunch, and reload.
10. Confirm points, allocation, effect, inventory, equipment, XP, and abilities persist.
11. Confirm no card popup appears anywhere in the normal level-up path.
12. Confirm no duplicated notification, modifier, or point award after reload/rebind.
