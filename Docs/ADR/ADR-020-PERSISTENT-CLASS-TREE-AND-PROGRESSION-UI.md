...
During the owner graphical playtest of PR #34, the owner reaffirmed that leveling should feel like a persistent action-RPG model: gain a point, then decide where to spend it in a dedicated class tree rather than being interrupted by a temporary randomized choice.
...
Primary attribute names, scaling, equipment requirements, and character-sheet presentation remain deferred pending the focused itemization prototype. ADR-020 concerns class progression points and class-tree allocation only.
...
All class trees use a consistent functional language even when their artwork differs.
...
Change how an active skill behaves. Mutation branches may be mutually exclusive unless an approved item, Law Node, or class rule breaks that restriction.

### 5.5 Law Nodes

Powerful rule changes with real tradeoffs. A Law Node changes priorities, sequencing, positioning, resource behavior, risk, or another governing rule rather than merely adding a large percentage.

**Law Node** is the shared AbyssFall functional term. Individual class trees may use a class-specific display title when approved, but implementation and documentation should not use another game's trademark-associated progression terminology as the durable shared label.

### 5.6 Culmination Nodes

Build-defining endpoints that complete a major progression direction. A class may use an approved class-specific display title while retaining the shared functional category.
...
The class's foundational identity and non-optional mechanic live at the board's primary origin or most visually dominant starting position.

### 6.2 Early progression

Readable early directions introduce the main play patterns without overwhelming a new player.

### 6.3 Transformation clusters

Build-changing mechanics, skill mutations, cross-cluster combinations, and serious specialization appear deeper in the commitment path.

### 6.4 Law and Culmination positions

High-commitment rule changes and defining endpoints occupy outer reaches or other visually significant terminal positions.
...
### 6.5 Non-radial commitment readability

A non-radial board must communicate progression depth and commitment as clearly as a radial board. Every composition prototype must use at least two independent visual channels, such as:

- path depth or travel distance from the origin;
- frame weight or node scale;
- connection thickness or path treatment;
- region boundaries or contour bands;
- icon hierarchy;
- escalating structural distortion;
- explicit tier/depth labeling.

A player must be able to answer, without opening a tooltip: **Where did I begin, what can I reach next, and which nodes represent major commitment?**

Each distinct composition type—not merely each class color palette—requires its own readability check.
...
2. Text and icons must remain readable at supported gameplay resolutions and UI-scale settings.
...
11. Safe areas, side panels, tooltips, point counters, and confirmation controls must remain usable across supported 16:9, 16:10, ultrawide, and fallback narrow/legacy aspect-ratio profiles.
12. The interface must be tested with common OS-level display scaling and in-game UI scaling; zooming the board may not be used as a substitute for readable text or reachable controls.
13. Controller focus and critical controls must remain visible when aspect ratio, resolution, or UI scale changes while the tree is open.

Exact supported resolution, aspect-ratio, and UI-scale matrices remain prototype outputs, but the prototype must test more than one ideal desktop configuration.
...
6. prove mouse and controller navigation, including a deliberately dense navigation stress layout;
7. prove save/reload, rebinding, and respec-safe contracts;
8. run graphical readability, aspect-ratio, UI-scale, and motion-comfort tests;
...
- exact supported resolution/aspect-ratio/UI-scale matrix;
...
ADR-020 grants design authority only.
...
- gives AbyssFall-native language to its highest-impact node categories;
...
- non-radial compositions require explicit commitment-depth language;
