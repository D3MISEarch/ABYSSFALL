...
### Integrated camera presentation (`scripts/integrated_camera_director.gd`)

`main.gd` owns the live camera node, the player reference, stable encounter facts, and every gameplay outcome. Its child `IntegratedCameraDirector` owns only temporary `Camera3D` transform/FOV presentation in the real `boot.gd` → `gameplay.tscn` → `full_stack_controller_main.gd` route. Default Gameplay is lower and closer than the former follow frame; Swarm Combat reads the existing `enemies_alive` count only in authored encounter states and uses explicit enter/exit thresholds plus an exit hold; the one-time Hollow King request captures the exact prior camera transform/FOV, frames the live player and boss, then restores that snapshot.

The director neither samples input nor owns hit validation, damage, health/death, rewards, progression, persistence, movement, collisions, class identity, AI, or encounter transitions. `main.gd` requests restoration on restart, boss death, and scene exit; the active `multiclass_main.gd` player-death override does the same. Invalid/dead reveal participants, interruption, reset, teardown, and scene changes therefore leave no camera-state drift and do not change gameplay.
